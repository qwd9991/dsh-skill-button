#!/bin/bash
# DSH-checkout build (compatibility variant). For the standalone/public build
# that only needs public npm packages, use `npm run build` instead.
# Requires DSH_CHECKOUT pointing at a dsh source checkout with a `packages/` dir.
set -euo pipefail
...
link_pkg react node_modules/.pnpm/node_modules/react
link_pkg @types/react node_modules/.pnpm/node_modules/@types/react
